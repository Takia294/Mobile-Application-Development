...
class _EmergencyRequestScreenState
  ...
  Future<void> submitRequest() async {
    ...
    try {
      ...
      // BUG FIX: previously nothing ever triggered a donor alert, so the
      // Notification screen's 'urgent' banner UI was dead — no request ever
      // produced one. High/Critical requests now broadcast to every donor.
      if (selectedUrgency == 'High' || selectedUrgency == 'Critical') {
        try {
          await NotificationService.sendBroadcast(
            type: 'urgent',
            title: requestType == 'Organ Donation'
                ? 'Urgent $selectedOrgan needed'
                : 'Urgent $selectedBloodGroup blood needed',
            subtitle: '$selectedHospital • $selectedUrgency urgency',
            buttonText: 'Respond Now',
          );
        } catch (_) {
          // Non-critical — the request itself was already saved successfully.
        }
      }
